...
<YYINITIAL>{
    ...
    {INTEGER} { words.add(new Word(yytext(), Constants.INTEGER, yyline+1, yycolumn+1));}

    {FLOAT}  {words.add(new Word(yytext(), Constants.REALNUMBER, yyline+1, yycolumn+1));}
...
<STRING> {
  \"                             { yybegin(YYINITIAL);words.add(new Word(string.toString(), Constants.STRING, yyline+1, yycolumn+1));}
