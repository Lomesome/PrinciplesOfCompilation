...
public class SingleFollow {
    ...
    public SingleFollow(Map<String, List<List<String>>> grammars, Map<String, Set<String>> firstSet){
    ...
    void getFollow(String c){
        ...
        for (List<String> s : list) {  //处理c的每一条产生式
            ...
            while (i >= 0 ) {
                String tn = s.get(i);
                if(grammars.containsKey(tn)){  //只处理非终结符
                    // 都按 A->αBβ  形式处理
                    //若β存在
                    if (s.size() - i - 1 > 0) {
                        List<String> right = s.subList(i + 1, s.size());
                        //把β的非空first集  加入followB
                        Set<String> setF = new LinkedHashSet<>();
                        for (String s1 : right){
